 ...
 * @since 1.0.0
 */
class Keynumbers_Image extends Widget_Base {
    ...
    public function get_name() {
        return 'keynumbers-image';
    }

    public function get_title() {
        return __( 'Bloc Chiffres clés / Image', 'assystem' );
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            'numbers',
            array(
                'label' => __( 'Chiffres clés', 'assystem' ),
                'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
            )
        );
    ...
    protected function render() {
        ...
        $image      = (!empty($settings['image']['url']))?$settings['image']:'';
        if( !empty($image) ) {
            $image  = wp_get_attachment_image_url($image['id'], 'keynumber-img');
        }
